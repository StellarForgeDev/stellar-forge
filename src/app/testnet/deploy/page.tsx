...
export default async function ControlledDeploymentPage() {
  const verification = await verifyCandidateArtifact(ACCESS_CONTROL_WORKFLOW.componentId);
  const isVerified = verification.status === "CANDIDATE_VERIFIED";
  ...
  const localHash = "actualHash" in verification ? verification.actualHash : "";
  const artifactPath = "candidate" in verification ? verification.candidate.artifactPath : "prebuilt/access-control.wasm";
  return (
    <main className="min-w-0 flex-1"><section className="mx-auto max-w-5xl px-6 py-16">
      ...
      <div className="mt-8 rounded-default border border-tone-onchain/40 bg-tone-onchain/5 p-5"><p className="font-mono text-xs uppercase tracking-[0.16em] text-accent-stellar">Eligibility is candidate-authorized</p><p className="mt-3 text-sm leading-6 text-text-secondary">Access Control is currently the only eligible candidate for controlled Testnet deployment. Its current candidate artifact is {isVerified ? "verified" : "not verified"}; this does not claim historical deployment.</p><p className="mt-3 text-xs leading-6 text-text-secondary">The deployment definition uses the catalog constructor signature. Constructor values, artifact hash, deployer, transaction hash, contract ID, and confirmation evidence will only be recorded after an independently confirmed deployment.</p></div>
      {accessControl && <ControlledDeploymentPanel artifactHash={localHash} artifactPath={artifactPath} artifactStatus={verification.status} connectivityHealthy={connectivity.status === "NETWORK_OK" || connectivity.status === "HEALTHY"} />}
