...
> the existing builder/validate/prepare/submit flow discovers Subscription
...
Testnet availability it cannot honor — Subscription's `testnet` flag is `true`
...
Subscription is in the Testnet flow: `capabilities.testnet` is `true`
